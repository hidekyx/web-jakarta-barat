	...
	<div class="content-wrapper">
		...
		<div class="content-body">
			<div class="card-header">
				<i class="fa fa-plus"></i> Edit Kecamatan
			</div>
			@foreach($data as $p)
			<form action="/kecamatanupdate/{{ $p->id }}" method="post" class="add" enctype="multipart/form-data">
				<div class="card-block">
					@csrf
					<label for="basic-url" class="form-label">Input Nama Kecamatan</label>
					<div class="input-group mb-3">
						<span class="input-group-text">Nama Kecamatan</span>
						<input type="text" class="form-control" required="required" disabled value="{{$p->nama}}" placeholder="Ketik disini" aria-label="Recipient's username" aria-describedby="basic-addon2" id="nama" name="nama">
					</div>
					<div>
						<span class="input-group-text">Profil Kecamatan</span>
					...
					<div class="form-group d-flex">
						...
						&nbsp;&nbsp;&nbsp;
						<a href="/kelurahan/{{$p->id}}" class="btn btn-primary">Update Kelurahan</a>
						&nbsp;&nbsp;&nbsp;
						<a href="/kecamatan" class="btn btn-submit btn-primary">Kembali</a>
					</div>
				</div>
			</form>
			@endforeach
