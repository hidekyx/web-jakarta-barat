...
<div class="content-wrapper">
    ...
    <div class="content-body">
        <div class="card-header">
            <i class="fa fa-plus"></i>Tolak Permohonan Informasi
        </div>
        @foreach($data as $d)
        <div class="card-block">
            <form action="/layanan-info-publik/permohonan/tolak/simpan/{{ $d->id_permohonan }}" method="post" class="add" enctype="multipart/form-data" autocomplete="off">
            @csrf
                <label for="basic-url" class="form-label">Alasan Penolakan</label>
                <div class="input-group mb-2">
                    <span class="input-group-text">Alasan</span>
                    <input type="text" class="form-control" required="required" placeholder="Ketik Disini" id="keterangan" name="keterangan">
                    <div class="input-group-append">
                        <button type="submit" class="btn btn-submit btn-primary">Tolak Permohonan</button>
                    </div>
                </div>
            </form>
            <hr>
