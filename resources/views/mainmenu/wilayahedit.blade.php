...
<div class="app-content content">
	...
	<div class="content-wrapper">
		...
		<div class="content-body">
			...
			@foreach($data as $p)
			<form action="/wilayah/update/{{ $p->id }}" method="post">
				<div class="card-block">
				@csrf
					<div>
						<span class="input-group-text">Isi {{ $p->nama }}</span>
						<textarea class="form-control" name="konten" id="tiny"> {!! $p->konten !!}</textarea>
					</div>
					<br><br>
					<div class="form-group">
						<button type="submit" value="Simpan Data" class="btn btn-submit btn-success">Edit</button>
						<a href="/wilayah" class="btn btn-submit btn-primary">Kembali</a>
					</div>
				</div>
      		</form>
      		@endforeach
