<!DOCTYPE html>
    ...
    <meta charset="utf-8">
        ...
        <title>Daftar Informasi Publik PPID - Kota Administrasi Jakarta Barat</title>
    </head>
    <script>
        $(document).ready(function(){
            $(document).on('change', '#onselectcat', function(){
                var value = $('#onselectcat').val();
                if(value == null || value == ""){
                        console.log('kosong');
                    } else{
                        window.location.href = '/daftar-info-publik/'+value;
                    }
            })
        })
    </script>
                ...
                <div class="row row mt20 mb30">
                    ...
                    <div class="col-md-6 text-right pt-5" style="text-align: right;">
                        <ul class="breadcrumbq">
                            ...
                            <span style="color:#fff">/</span>
                            <span class="active">Daftar Informasi Publik</span>
                            </li>
                        </ul>
                    </div>
                </div>
                    ...
                    <br>
                        <p class="blogq" style="color: #747474;">
                            <h4 style="box-sizing: border-box; outline: none; font-family: Montserrat, sans-serif; line-height: 30px; margin-top: 10px; margin-bottom: 10px; font-size: 18px; -webkit-font-smoothing: antialiased; background-color: #ffffff; font-weight: normal !important;"><span style="box-sizing: border-box; outline: none; font-weight: bold;">
                                Daftar Informasi Publik
                                PPID Kota Administrasi Jakarta Barat
                            </span></h4>
                        </p>
                        <div>
                            <select class="form-control" style="width: 30%;" id="onselectcat">
                        ...
                        <table class="table table-striped" id="tabel-data">
                            ...
                            <tr>
                                <td>{{ $item->nomor }}</td>
                                <td>{{ $item->nama_inf }}</td>
                                <td>{{ $item->penanggung_jaw }}</td>
                                <td>{{ $item->kategori }}</td>
                                <td class="text-center">
                                    @if ($item->file != null)
                                    <!-- <a class="btn btn-primary" target="blank" href="{{ asset('storage/ppid_docs/'.$item->file) }}">Download</a> -->
                                    <form method="post" action="{{ asset('/ppid-download/'.$item->id_dftr) }}" enctype="multipart/form-data">
                                    @csrf
                        ...
                        <br>
                        {{ $data->links() }}
                        <p>Jumlah Download: {{ $jumlah_download }}</p>
