<main id="main" class="main">
    <div class="pagetitle">
        ...
        <nav>
            ...
            @if($subpage == "tugas-dan-fungsi-ppid")
    ...
    <div class="row">
        ...
        <form class="row needs-validation" id="form-ppid" method="post" action="{{ asset('dashboard/ppid/'.$subpage.'/simpan') }}" enctype="multipart/form-data" novalidate>
        @csrf
            ...
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-body">
                        @if($subpage == "tugas-dan-fungsi-ppid")
                            <h5 class="card-title">Tugas dan Fungsi PPID - {{ $logged_user->nama }}</h5>
                        @elseif($subpage == "visi-dan-misi-ppid")
                            <h5 class="card-title">Visi dan Misi PPID - {{ $logged_user->nama }}</h5>
                        @else
                            <h5 class="card-title">@unlink($subpage) - {{ $logged_user->nama }}</h5>
                        @endif
                        <div class="quill-editor-default" id="ppid" style="height: 50vh;">
                        @if($data_ppid)
                            {!! $data_ppid->konten !!}
                        @endif
                        </div>
            ...
            <div class="col-lg-4 section profile">
                <div class="card">
                    <div class="card-body">
                        @if($subpage == "tugas-dan-fungsi-ppid")
                            <h5 class="card-title">Status Data Tugas dan Fungsi PPID</h5>
                        @elseif($subpage == "visi-dan-misi-ppid")
                            <h5 class="card-title">Status Data Visi dan Misi PPID</h5>
                        @else
                        ...
                        <div class="tab-pane fade show active profile-overview">
                            ...
                            <div class="row">
                                ...
                                @if($subpage == "tugas-dan-fungsi-ppid")
                                    <div class="col-lg-9 col-md-8">{{ $page }} - Tugas dan Fungsi PPID</div>
                                @elseif($subpage == "visi-dan-misi-ppid")
                                    <div class="col-lg-9 col-md-8">{{ $page }} - Visi dan Misi PPID</div>
                                @else
                            ...
                            <div class="row">
                                ...
                                @if($data_ppid)
                                    @if($data_ppid->status == "Menunggu Konfirmasi")
                                    <span class="badge bg-warning">Menunggu Konfirmasi</span>
                                    @elseif($data_ppid->status == "Sudah Dipublikasi")
                            ...
                            $("#submit-form").click(function(e){
                                var myEditor = document.querySelector('#ppid');
                                ...
                                if (hasText) {
                                    ...
                                    iziToast.error({
                                        title: 'Gagal!',
                                        message: 'Data {{ $submenu }} harus anda isi terlebih dahulu!',
                                        overlay: true,
                                        position: 'topRight',
                                    });
                                }
