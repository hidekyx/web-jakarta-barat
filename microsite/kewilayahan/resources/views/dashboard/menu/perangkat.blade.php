<main id="main" class="main">
    <div class="pagetitle">
        ...
        <nav>
            ...
            @if($subpage == "lmk")
                <li class="breadcrumb-item active">{{ strtoupper($subpage) }}</li>
            @else
    ...
    <div class="row">
        ...
        <form class="row needs-validation" id="form-perangkat" method="post" action="{{ asset('dashboard/perangkat/'.$subpage.'/simpan') }}" enctype="multipart/form-data" novalidate>
        @csrf
            ...
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-body">
                        @if($subpage == "lmk")
                            <h5 class="card-title">{{ strtoupper($subpage) }} {{ $logged_user->nama }}</h5>
                        @else
                            <h5 class="card-title">@unlink($subpage) {{ $logged_user->nama }}</h5>
                        @endif
                        <div class="quill-editor-default" id="perangkat" style="height: 50vh;">
                        @if($data_perangkat)
                            {!! $data_perangkat->konten !!}
                        @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-4 section profile">
                <div class="card">
                    <div class="card-body">
                        @if($subpage == "lmk")
                        <h5 class="card-title">Status Data {{ strtoupper($subpage) }}</h5>
                        @else
                        ...
                        <div class="tab-pane fade show active profile-overview">
                            ...
                            <div class="row">
                                ...
                                @if($subpage == "lmk")
                                <div class="col-lg-9 col-md-8">{{ $page }} - {{ strtoupper($subpage) }}</div>
                                @else
                            ...
                            <div class="row">
                                ...
                                @if($data_perangkat)
                                    @if($data_perangkat->status == "Menunggu Konfirmasi")
                                    <span class="badge bg-warning">Menunggu Konfirmasi</span>
                                    @elseif($data_perangkat->status == "Sudah Dipublikasi")
                            ...
                            $("#submit-form").click(function(e){
                                var myEditor = document.querySelector('#perangkat');
