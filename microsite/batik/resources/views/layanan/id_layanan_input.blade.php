...
<div class="sc-contact-section sc-pt-100 sc-md-pt-70 sc-pb-80 sc-md-pb-50">
    <div class="container">
        ...
        <div class="sc-heading-area text-center sc-pr-30 sc-pt-0 sc-pb-30">
            ...
            <h3 class="title">
                Infrastuktur <span class="primary-color italic">Digital</span>
            </h3>
        </div>
        ...
        @endif
        <form role="form" action="{{ asset('/layanan-id/form/submit'); }}" method="post" enctype="multipart/form-data" autocomplete="on">
        @csrf
            <div class="row">
                <div class="col-lg-4">
                    <div class="sc-process-system-three sc-sm-mb-50">
                        <div class="process-content">
                            ...
                            <select title="Pilih Layanan" data-size="11" class="form-control form-select selectpicker" name="kategori" id="kategori" required="" onchange="changeFunc();">
                                ...
                                <optgroup label="Infrastuktur Digital">
                ...
                function changeFunc() {
                    ...
                    $('#id_instalasi').fadeOut("fast");
                    $('.instalasi_id_form').prop("required", false);
                    $('#id_penanganan').fadeOut("fast");
                    $('.penanganan_id_form').prop("required", false);
                    $('#id_zoom').fadeOut("fast");
                    $('.zoom_id_form').prop("required", false);
                    if(kategori_value == "Instalasi, Penambahan, dan Penataan Jaringan") {
                        $('#id_instalasi').delay(100).fadeIn();
                        $('.instalasi_id_form').prop("required", true);
                        $('#deskripsi_layanan').html('Layanan terhadap Pengembangan Jaringan Internet dan Penggunaan Akses Intranet berdasarkan Sistem Komunikasi intra Pemerintah Provinsi DKI Jakarta');
                    }
                    else if(kategori_value == "Penanganan Permasalahan Jaringan") {
                        $('#id_penanganan').delay(100).fadeIn();
                        $('.penanganan_id_form').prop("required", true);
                        $('#deskripsi_layanan').html('Dukungan teknis permasalahan Jaringan Internet dan Penggunaan Akses Intranet berdasarkan Sistem Komunikasi intra Pemerintah Provinsi DKI Jakarta');
                    }
                    else if(kategori_value == "Dukungan Zoom Meeting") {
                        $('#id_zoom').delay(100).fadeIn();
                        $('.zoom_id_form').prop("required", true);
                        $('#deskripsi_layanan').html('Dukungan konfigurasi perangkat Video Conference serta koneksi jaringan pada lokasi kegiatan. (Tim Teknis hanya dilengkapi perangkat Laptop dan Kamera)');
                    }
                }
                </script>
                @include('layanan.id_instalasi')
                @include('layanan.id_penanganan')
                @include('layanan.id_zoom')
                <script type="text/javascript">
