<div class="col-12 mt-4">
    ...
    <div class="card-header pb-0 px-3">
        <div class="row">
        <div class="col-md-6">
            <h6 class="mb-0">Keterangan Revisi - {{ $k->user->nama_lengkap }} </h6>
        </div>
        
        </div>
    </div>
    <div class="card-body pt-4 p-3">
        <form role="form" class="text-start" action="{{ asset('/kegiatan/generate_revisi/'.$k->user->username.'/'.$k->id_kegiatan); }}" method="post" enctype="multipart/form-data" autocomplete="off">
        @csrf
        <div class="row px-2">
            @if(session('errors'))
        ...
        <div class="row form-libur">
            <div class="col-lg-2">
                ...
                <div class="input-group input-group-outline mb-3">
                    <input disabled type="date" class="form-control" value="{{ $k->tanggal }}" required>
                </div>
            </div>
            
            <div class="col-lg-10">
                <h6 class="text-dark text-sm ">Deksiprsi Kegiatan: </h6>
                <div class="input-group input-group-outline mb-3">
                    <input disabled type="text" class="form-control" value="{{ $k->deskripsi }}" required>
                </div>
            </div>     
            <div class="col-lg-12">
                <h6 class="text-dark text-sm ">Keterangan Revisi: </h6>
                <div class="input-group input-group-outline mb-3">
                    <input name="keterangan" type="text" class="form-control" value="{{ $k->keterangan }}" maxlength="255">
                </div>
            </div>          
        </div>
        </div>
